 ...
 *
 * WHY THE SERVER IS COPIED OUT OF THE BUNDLE. `lib/browser/flow-spec.js` and
 ...
 * electron-builder copies `product-ext/lib/flow-mcp/` and those two modules,
...
function candidates() {
    ...
    list.push(path.resolve(cwd, 'product-ext', 'lib', 'flow-mcp', SERVER_FILE));
    list.push(path.resolve(cwd, 'app', 'product-ext', 'lib', 'flow-mcp', SERVER_FILE));
    list.push(path.resolve(cwd, '..', 'app', 'product-ext', 'lib', 'flow-mcp', SERVER_FILE));
    return list;
}
