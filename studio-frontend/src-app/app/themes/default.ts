...
export const defaultTheme: ThemeConfig = {
  ...
  variables: {
    ...
    // Raised surface shown translucent on hover and opaque when active, plus the
    // strong foreground both engaged states share. The accent on an active
    // item's icon is the brand --primary, not a menu-local copy of it.
    ...
    // Typography. Same token name and stack as @gears-frontx/ui-kit's
    // --font-sans, so shell chrome and MFE screens read as one product.
    // The font FILES are the app's build concern — see globals.css.
    '--font-sans': "'Inter Variable', ui-sans-serif, system-ui, -apple-system, 'Segoe UI', sans-serif",
    '--text-body-size': '0.9375rem',       // Body 15/20
    '--text-body-line-height': '1.25rem',
    '--text-heading-1-size': '1.25rem',    // Heading 1 20/28
    '--text-heading-1-line-height': '1.75rem',
    '--text-label-size': '0.8125rem',      // Label 13/16
